...
<body class="relative bg-blue-50 overflow-x-hidden font-sans">
            ...
            <div class="md:hidden">
                <div class="flex items-center justify-between gap-2 mb-3">
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <!-- Carrito -->
                        <a href="{{ route('carrito.index') }}" title="Carrito de compras"
                           class="relative bg-gray-100 hover:bg-gray-200 px-2 py-2 rounded-lg transition">
                            ...
                            <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 z-50">
                                @auth
                                    <a href="{{ Auth::user()->rol_id == 1 ? route('admin.dashboard') : route('cliente.dashboard') }}" 
                                       class="block px-4 py-2 text-gray-800 hover:bg-blue-100 text-sm font-medium">
                                          Mi Panel
                                    </a>
                ...
                <form method="GET" action="{{ route('tienda') }}" class="flex">
                    <input type="text" name="buscar"
                           ...
                           class="flex-1 border-0 rounded-l-lg px-3 py-2 text-sm text-gray-800 focus:ring-2 focus:ring-blue-400 focus:outline-none">
            ...
            <div class="hidden md:flex items-center justify-between gap-6">
                ...
                <form method="GET" action="{{ route('tienda') }}" class="flex-1 max-w-md">
                    <div class="flex">
                        <input type="text" name="buscar"
                               placeholder="🔍 Buscar producto..."
                               value="{{ request('buscar') }}"
                               class="flex-1 border-0 rounded-l-lg px-4 py-2 text-gray-800 focus:ring-2 focus:ring-blue-400 focus:outline-none">
                ...
                <div class="flex items-center gap-4 flex-shrink-0">
                    @auth
                        <!-- Bienvenido usuario -->
                        <a href="{{ Auth::user()->rol_id == 1 ? route('admin.dashboard') : route('cliente.dashboard') }}"
                           class="text-white font-semibold hover:text-blue-200 transition text-sm">
                             {{ Auth::user()->nombres }}
                        </a>
                    ...
                    <a href="{{ route('carrito.index') }}"
                       class="relative bg-gray-100 hover:bg-gray-200 px-4 py-2 rounded-lg ml-2 transition" title="Carrito de compras">
                        🛒
                        @php
                            $cantidad = count(session('carrito', []));
                        @endphp
    ...
    <footer id="footer" class="bg-[#004080] text-white py-8 md:py-10 px-4 md:px-8 mt-10 rounded-lg md:rounded-xl m-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
        ...
        <div>
            ...
            <p class="text-xs md:text-sm opacity-80 mb-1 break-all">phoneneus@gmail.com</p>
            <p class="text-xs md:text-sm opacity-80">3209643887</p>
        </div>
